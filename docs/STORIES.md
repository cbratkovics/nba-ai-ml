...
| Drift | Calibrated on 164 replay dates by position: PSI ≥ 0.15 on three features, zero false positives (0.05 would have fired on 69 dates); three injected shifts HOLD, one WARNs. | `reports/drift_calibration_2025-26.json` `chosen`, `false_positives_total`, `sensitivity`; ADR-0018 |
...
| Operations | Pass rates over repeated runs are measured by a spreader one golden date per day, recorded with the golden-set version, and rendered into the README from the report; none complete yet under version 2. | `reports/agent_pass_rates.json` `overall`; `agent-eval.yml` |
| Cost | Runtime is $0: GitHub Actions, Hugging Face, Vercel, Groq free tier, MotherDuck free tier; no server, no database on the serving path. | `README.md` architecture; ADR-0003 |
